...
const Home = () => {
    ...
    const iWillDoIt = async (task) => {
        await axios.post('/api/task/updatetask', {id: task.id, name: task.name })
    }

    const itIsDone = async (task) => {
        await axios.post('/api/task/deletetask', { id: task.id, name: task.name, userId: user.id})
    }
                            ...
                            {tasks.map(t => (
                                <tr key={t.id}>
                                    ...
                                    <td>
                                        ...
                                        {!t.userId && <button className="btn btn-dark" onClick={()=>iWillDoIt(t)}>I'm doing this one!</button>}
                                        {t.userId === user.id && <button className="btn btn-success" onClick={()=>itIsDone(t)}>I'm done!</button>}
                                </td>
                            </tr>
                            )) }
